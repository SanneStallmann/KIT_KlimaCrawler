...
def ensure_seed_job_events_schema(con: sqlite3.Connection) -> None:
    con.executescript(
        ...
        CREATE TABLE IF NOT EXISTS seed_job_events (
          ...
          worker_id       TEXT,
          event           TEXT NOT NULL,  -- claim|heartbeat|done|failed|reclaim
          details         TEXT
        );
...
def _log_event(
    ...
    # best-effort logging; should never break crawling
    try:
        con.execute(
            """
...
def claim_next_job(
    ...
    """
    ensure_seed_job_events_schema(con)
...
def heartbeat_job(con: sqlite3.Connection, municipality_id: str, worker_id: str) -> None:
    ...
    _log_event(con, municipality_id, worker_id, "heartbeat", None)
